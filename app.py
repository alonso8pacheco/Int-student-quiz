...
@ app.route('/yourscore', methods=["POST", "GET"])
def yourscore():

    if request.method == "POST":
        if request.form['submit_button'] == 'Play Again':
...
def check_answer(user_answer, desired_pos, question_id, user_id, correct, seen):
    ...
    if str(user_answer) == str(answers[desired_pos]):
        ...
        if seen == 0:
            ...
            update_new_correct(question_id, user_id)
        else:
            session["count_correct"] += 1
        ...
        if seen == 0:
            update_new_incorrect(question_id, user_id)
        else:
...
def get_current_user():
    ...
    if 'user' in session:
        ...
        if "count" not in session:
            session["count"] = 0
        if "count_correct" not in session:
            session["count_correct"] = 0
...
def update_new_correct(question_id, user_id):
    conn = db.connect('quizapp.db')
    conn.row_factory = lambda cursor, row: row[0]
    cn = conn.cursor()

    # make a new row in accuracy
...
def update_new_incorrect(question_id, user_id):
    conn = db.connect('quizapp.db')
    conn.row_factory = lambda cursor, row: row[0]
    cn = conn.cursor()
...
def displayquestion():  # makes the form and a new function receives the form results
    ...
    user = get_current_user()
    if session['count'] == 5:
        correct_score = session['count_correct']
        session['count'] = 0
        session['count_correct'] = 0
        if correct_score/3 < .5:

            return render_template('score_fail.html', correct=correct_score, count=session['count'])
        else:

            return render_template('score_pass.html', correct=correct_score, count=session['count'])
    # for i in range(10):
    #    count = i
    i = 0
    while i < 10:
        ...
        # if the question is not in the accuracy table
        # ask it and
        # add it there
        # will have its own retur statement
        ...
        if exists[0] == 0:
            ...
            return render_template("displayquestion.html",
                                   ...
                                   answer_a=answer_a, count_correct=session['count_correct'],
                                   answer_b=answer_b, answer_c=answer_c,
                                   answer_d=answer_d, question_id=question_id,
                                   user=user, count=session["count"])
        ...
        if accuracy < .6:
            ...
            return render_template("displayquestion.html",
                                   ...
                                   answer_d=answer_d, question_id=question_id,
                                   user=user, count=session["count"],
                                   count_correct=session['count_correct'],)
        else:
            continue
    # after question 10
